...
当前版本：`v0.1.57`，`v0.1.38` 以下历史版本无密钥，若使用新版请联系作者。

## 下载与安装

- [下载 Format Flow v0.1.57 Windows 安装包](https://github.com/Given-Dream/format-flow/releases/download/v0.1.57/Format-Flow-Setup-0.1.57.exe)
...
变量填充还可以读取桌面 Word 当前选区，或接收拖入/本地选择的 Word、图片、文本及其他文件，整理为临时 `.docx`。提示词变量和文档页首使用同一个 `FFV-xxxxxx` 附件 ID，避免长提示词中的附件位置错配；Word 选区中的 OMML 公式、表格、图片与格式会保留。临时文档支持打开、定位、复制文件和删除，默认保留 24 小时并定期清理。
